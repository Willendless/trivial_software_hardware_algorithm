...
```shell
$ g++ mmm.c
$ ./a.out [input file] [0/1/2] > output.txt
```
...
输出为10000次mmm计算的模拟。其中三元组`<c, k, K>`，分别表示
...
1. 使用线性/动态规划离线确定硬件和软件实现的任务。
    + **目标**: 在硬件面积的约束下，使得硬件增益最大。
    + **动态规划求解**
      + 设dp[i][k]为考虑前i个任务且硬件面积约束为k的情况下能获得的最大硬件增益。第i个任务的硬件面积为wi，硬件增益为ai。
      + **定义**
        + dp[i][k] = max(dp[i-1][k-wi] + ai, dp[i-1][k])
...
```c
11 2 20  // 11个总任务, 执行者数为2（一个硬件一个软件），硬件面积的限制为20
2 // 第一个执行者: 2个核（默认为第一个执行者为软件!）
2 // 第二个执行者: 2个核
4 2 4 0 // 任务1: 第一个执行者执行时间为4，第二个执行者执行时间为2，硬件面积为4，释放时间为0
5 3 2 0 // 任务2: （下同）
4 1 4 0
5 2 7 0
3 1 5 0
